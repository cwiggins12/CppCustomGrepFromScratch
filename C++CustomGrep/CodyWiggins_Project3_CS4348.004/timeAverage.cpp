...
int main() {
	vector<int> bufferSizes = { 1, 5, 10, 15, 20, 25, 30, 35, 40, 50, 60 };
	vector<int> times;
  int runPerSize = 500;
	string fileSize = "-1";
	string uid = "-1";
	...
	for (int size : bufferSizes) {
		for (int i = 0; i < runPerSize; i++) {
			string sizeStr = to_string(size);
			const char* argv[] = { "pipegrepWithTimers", sizeStr.c_str(), fileSize.c_str(), uid.c_str(), gid.c_str(), word.c_str() };
			int time = run(6, const_cast<char**>(argv));
	...
	for (int i = 0; i < bufferSizes.size(); i++) {
		int sum = 0;
		for (int j = 0; j < runPerSize; ++j) {
			int val = times[i * runPerSize + j];
			sum += val;
		}
		double avg = static_cast<double>(sum) / runPerSize;
		cout << "Average of Buffer Size " << bufferSizes[i] << ": " << fixed << setprecision(3) << avg << " ms" << endl;
	}
